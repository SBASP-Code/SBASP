...
ofstream mycout01("//scratch//lxwu//2026//SBASP//Results//A2//Solution//B" + num2str(B) + "_S" + num2str(S) + "_CN" + num2str(CNN) + "_Code_" + num2str(CODE) + ".txt");
ofstream mycout02("//scratch//lxwu//2026//SBASP//Results//A2//Computational//B" + num2str(B) + "_S" + num2str(S) + "_CN" + num2str(CNN) + "_Code_" + num2str(CODE) + ".txt");
...
double mastersolver02(double thelb) { 
	double lwbd;
...
void PHeuristics() {
	...
	for (int nn = 0; nn != numused; ++nn) {
		int theb = colberthused[nn];
		double theval = colsolused[nn];
		if (Berthmax[theb] < theval) {
			Berthmax[theb] = theval;
			Berthind[theb] = nn;
		}
	}
	int TBVN[B];
	...
	for (int b = 0; b != B; ++b) {
		TBVN[b] = 0;
		int thecol = Berthind[b];
		int thevn = colvnused[thecol];
		if (thecol == -1) { continue; }
		for (int vn = 0; vn != thevn; ++vn) {
			int theves = colvesused[thecol][vn];
			if (Vescover[theves] == 0) {
				TBVN[b] += 1;
				TVN += 1;
				double thest = AVEEAtime[theves]; 
				if (thest < CBRT[b]) { 
				...
				CBRT[b] = thest + avetime[theves][b];
				BVset[b].resize(TBVN[b], theves);
				Vescover[theves] = 1;
			}
		}
	}
	if (TVN < I) { 
		...
		for (int i = 0; i != I; ++i) {
			...
			TBVN[theberth] += 1;
			BVset[theberth].resize(TBVN[theberth], theves);
			double thest = AVEEAtime[theves];
			if (thest < CBRT[theberth]) {
				thest = CBRT[theberth];
			}
	...
	for (int cn = 0; cn != NBC; ++cn) {
		double thecost = CBpara[cn];
		for (int i = 0; i != TSN; ++i) {
			int thei = TSset[i]; 
			thecost += Zsol[thei] * ZBpara[i][cn];
		}
		for (int b = 0; b != B; ++b) {
			for (int vn = 0; vn != TBVN[b]; ++vn) {
				int theves = BVset[b][vn];
				thecost += XBpara[theves][b][cn];
				if (vn != TBVN[b] - 1) {
					for (int fvn = vn + 1; fvn != TBVN[b]; ++fvn) {
						int thefv = BVset[b][fvn];
						thecost += YBpara[b][theves][thefv][cn];
					}
				}
			}
		}
		int thes = BCS[cn];
		if (BDScost[thes] < thecost) {
			BDScost[thes] = thecost;
		}
	}
	Fcost = 0;
	double Bdcost = 0;
	for (int s = 0; s != S; ++s) {
		Bdcost += Pro[s] * BDScost[s];
	}
	for (int b = 0; b != B; ++b) {
		for (int i = 0; i != I; ++i) {
			Xsol[i][b] = 0;
	...
	for (int b = 0; b != B; ++b) {
		for (int vn = 0; vn != TBVN[b]; ++vn) {
			int theves = BVset[b][vn];
			Xsol[theves][b] = 1;
			Fcost += Hcost[theves][b];
			if (vn != TBVN[b] - 1) {
				for (int fvn = vn + 1; fvn != TBVN[b]; ++fvn) {
					int thefv = BVset[b][fvn];
					Ysol[b][theves][thefv] = 1;
				}
			}
		}
	}
	for (int cn = 0; cn != TSN; ++cn) {
		Fcost += Hsol[cn];
	}
	double thelb = Fcost + Bdcost;
	Colclear();
	USEDColclear();
	for (int b = 0; b != B; ++b) {
		if (TBVN[b] > 0) {
			numused += 1;
			colberthused.resize(numused, b);
			colvnused.resize(numused, TBVN[b]);
			colvesused.resize(numused);
			double tempadcost[S]; 
			double tempcrttime[S]; 
			for (int s = 0; s != S; ++s) {
				tempadcost[s] = 0;
				tempcrttime[s] = br[b][s];
			}
			double tempcost = 0;
			for (int vn = 0; vn != TBVN[b]; ++vn) {
				int theves = BVset[b][vn];
				colvesused[numused - 1].resize(vn + 1, theves);
				tempcost += Hcost[theves][b];
				for (int s = 0; s != S; ++s) {
					double thest = tempcrttime[s];
					if (thest < EAtime[theves][s]) {
						thest = EAtime[theves][s];
					}
					double theet = thest + Htime[b][theves][s];
					tempcrttime[s] = theet;
					double thedelay = (theet - Ltime[theves][s]) - Vtime[theves];
					if (thedelay < 0) { thedelay = 0; }
					double tempdc = Tcost[theves] * pow(double(thedelay), double(SLP));
					double thestay = Htime[b][theves][s];
					if (theet - (Ltime[theves][s] + Stime[theves][1]) > thestay) {
						thestay = theet - (Ltime[theves][s] + Stime[theves][1]);
					}
					tempdc += Ecost[theves][1] * Gpara[theves] * thestay;
					tempadcost[s] += tempdc;
				}
			}
			colcostused.resize(numused, tempcost);
			for (int s = 0; s != S; ++s) {
				coladcostused[s].resize(numused, tempadcost[s]);
			}
		}
	}
	int themfrc = 1;
	while (themfrc == 1 && thelb <= upperbound - 0.001) {
		thelb = mastersolver02(thelb); 
		themfrc = Fuelsep(thelb); 
	}
	for (int i = 0; i != I; ++i) {
	...
	double bdub = tempub + Rfgap;
	if (upperbound > bdub) {
		upperbound = bdub;
		cout << "new upper bound by hueristics" << upperbound << endl;
		BSrecord();
	}
	double tempgap = (bdub - (thelb + Rfgap)) / bdub * 100;
	if (tempgap > BDgap02) {
		BDsep();
	}
	while (tempgap > BDgap02 && thelb <= upperbound - 0.001) {
		thelb = mastersolver02(thelb); 
		themfrc = 1;
		while (themfrc == 1 && thelb <= upperbound - 0.001) {
			thelb = mastersolver02(thelb); 
		...
		tempgap = (bdub - (thelb + Rfgap)) / bdub * 100;
		if (tempgap > BDgap02) {
...
void Benders(int lel, int ind, int branch, double thelb) {
	...
	if (intsol == 1 && bdlb <= upperbound - 0.001) {
		...
		while (tempgap > BDgap02 && bdlb <= upperbound - 0.001) {
			themfrc = 1;
			double bdlb02 = bdlb;
			while (themfrc == 1 && bdlb02 <= upperbound - 0.001) {
				bdlb02 = mastersolver02(bdlb02); 
				themfrc = Fuelsep(bdlb02); 
			}
			...
			if (upperbound > bdub) {
				upperbound = bdub;
				cout << "new upperbound by mastersolver02=" << upperbound << endl;
				BSrecord();
			}
			tempgap = (bdub - (bdlb02 + Rfgap)) / bdub * 100;
			if (tempgap > BDgap02) {
